...
class Distributor
{
    ...
    private function distributeProcess($metadataRuleDiscountItem)
    {
        $childrenDiscount = [];
        $item = $metadataRuleDiscountItem->getItem();
        $qtyToDiscount = $metadataRuleDiscountItem->getQty();
        $parentBaseRowTotal = $this->processor->getTotalItemBasePrice($item, $qtyToDiscount);
        foreach ($item->getChildren() as $child) {
            $ratio = $this->processor->getTotalItemBasePrice($child, $qtyToDiscount) / $parentBaseRowTotal;
